...
func (c *Controller) Register(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case err != nil:
		log.Error("failed to process the request", "error", err.Error())
		http.Error(w, er.ErrDecodeJSON.Error(), http.StatusBadRequest)
		return
	case len(user.Login) == 0:
		log.Error("error: empty login", "error", err, "login", user.Login)
	...
	if err != nil {
		log.Error("can't set cookie: ", "error controllers|register", err.Error())
		http.Error(w, er.ErrInternalServer.Error(), http.StatusInternalServerError)
		return
	}
