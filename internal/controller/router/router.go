...
func SetupRouter(handler *chi.Mux, log *slog.Logger, useCase *usecase.UseCase, er *usecase.ErrStatus, entity *entity.Entity) *chi.Mux {
	handler.Use(middleware.RequestID)
	handler.Use(logger.New(log))
	handler.Use(middleware.Logger)
	handler.Use(gzip.New())

	h := controller.New(useCase, log, er, entity)
